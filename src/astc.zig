...
//!
//! This module is pure byte-parsing — NO raylib/GPU dependency — so the format
//! handling is host-testable. The backend maps `block_x`/`block_y` to a raylib
//! `PixelFormat` and uploads `blocks` directly (zero CPU decode) via raylib's
//! `loadTextureFromImage`, which routes the compressed format through
//! `rlLoadTexture` → `glCompressedTexImage2D`. This is the only zero-cost upload
//! path for a 4K atlas (see labelle-gfx#269 / #341).
